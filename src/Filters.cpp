...
//std::shared_ptr<std::vector<float>> getRampFilter(size_t iSize)
//{
//	std::cout << "=== getRampFilter called with iSize=" << iSize << " ===" << std::endl;
//
//	// 方法1：直接测试mutex构造
//	std::cout << "Testing mutex construction..." << std::endl;
//	try {
//		static std::mutex static_mutex;
//		std::cout << "Mutex constructed at: " << &static_mutex << std::endl;
//
//		std::cout << "Attempting to lock..." << std::endl;
//		std::unique_lock lock{ static_mutex };  // 你的第89行
...
//std::shared_ptr<std::vector<float>> getRampFilter(size_t iSize)
//{
//	static std::atomic<size_t> cached_size{ 0 };
//	static std::vector<float> cached_filter;
//	static std::atomic<bool> initialized{ false };
//
//	// 快速路径：检查是否已经缓存了所需尺寸
//	size_t current_size = cached_size.load(std::memory_order_acquire);
//	if (initialized.load(std::memory_order_acquire) && current_size == iSize) {
//		return std::make_shared<std::vector<float>>(cached_filter);
//	}
//
//	// 慢速路径：需要生成新滤波器
//	auto new_filter = generateRampFilter(iSize);
//
//	// 更新缓存
//	cached_filter = new_filter;
//	cached_size.store(iSize, std::memory_order_release);
